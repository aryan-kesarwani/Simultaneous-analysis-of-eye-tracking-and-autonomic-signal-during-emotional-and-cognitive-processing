...
%
%  modified (4/17/20) Andreas Widmann
%  Made epsilon setting of Geomin rotation controllable via RotOpt input parameter.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for repetition =1:NumReps
    switch Rotation
        case 'Quartimax'  %Quartimax
            T=randorth(NumFacs);
            [FacPat,Th,table,f]=GPForth(A,T,Rotation);
            Phi = diag(ones(size(A,2),1));
        case {'Quartimin', 'Oblimin', 'Crawford-Ferguson', 'minimum entropy', 'invariant pattern simplicity', 'tandem II criterion', 'Geomin', 'McCammon minimum entropy'}
            T=randorth(NumFacs);
...
for iter=0:500
    ...
    for i=0:10
       ...
       switch Rotation
           ...
           case 'Geomin'
               [ft,Gq]=GMIN(L, RotOpt);
%            case 'SMAX'  I commented out the subroutine and don't remember why any more.
%                [ft,Gq]=SMAX(L);
           case 'McCammon minimum entropy'
               [ft,Gq]=MMER(L);
       end
...
%geomin
%--------------------------------------------------------------
function [q,Gq]=GMIN(L,RotOpt)

if nargin < 2 || isempty(RotOpt)
    ep=.01;
else
    ep=RotOpt;
end
