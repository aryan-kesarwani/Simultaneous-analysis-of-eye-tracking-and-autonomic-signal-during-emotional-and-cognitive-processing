function [PmxPat, phi] = ep_doPromax(VmxPat, k, theAlgorithm)

%[PmxPat, phi] = ep_doPromax(VmxPat, k, theAlgorithm)  - Compute promax rotation for PCA
...
%  theAlgorithm : implementation according to SAS or SPSS.
%
%Outputs
...
switch theAlgorithm
    case 'SAS'
        ...
        % Generates same results as SAS 9.4.  Algorithm undocumented but based on information from SAS Support (personal communication, 2000).
    ...
    case 'SPSS'
        ...
        % Algorithm according to IBM SPSS Statistics Algorithms Manual (2020).  Generates same results as SPSS 22.
        
        P = diag(1./sqrt(sum(VmxPat'.^2)))*VmxPat;	%Normalize rows - equalize sum of squares of each variable's loadings

        P = P.^k;		%compute target matrix by taking higher power of factor loadings

        P = abs(P) .* sign(VmxPat);		%add the signs back to the target matrix since even powers eliminate them

        L = inv(VmxPat'*VmxPat) * VmxPat' * P;

        D = diag(1./(sqrt(diag(L' * L))));

        Q = L*D;

        C = diag(1./(sqrt(diag(inv(Q' * Q)))));

        PmxPat = VmxPat * Q * inv(C);
