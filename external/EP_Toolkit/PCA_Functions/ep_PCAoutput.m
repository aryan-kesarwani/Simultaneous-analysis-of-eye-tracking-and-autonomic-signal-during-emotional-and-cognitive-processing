function [data] = ep_PCAoutput(FactorResults, params, paramNames, addAdds, missingData)
% [data] = ep_PCAoutput(FactorResults, params, paramNames, addAdds, missingData)
% outputs PCA results in EP format
%Inputs
...
%  addAdds    : Flag to add adds (1 = add)
%  missingData : value for missing data
%
%Outputs
...
%
% modified 8/6/18 JD
% Changed name of "all" subject and cell to "combined" to reduce confusion with '-all' option in View function.
%
% modified 4/9/19 JD
...
%
% bugfix 6/10/19 JD
% Fixed crash for PCA data prior to version 2.76.
%
% modified 11/4/19 JD
...
% 
% modified 3/27/20 JD
% Added option to turn off adds.
%
% bugfix 8/4/21 JD
% Fixed crash when using parametric option.
% Missing data values for parametric option conforms to preferences setting.
% 
% modified 10/13/21 JD
...
numOutputCells=length(FactorResults.cellNames);
...
data.cellNames=FactorResults.cellNames;
...
data.sessNames=FactorResults.sessNames;
...
data.sessNums=FactorResults.sessNums;

data.Fs=FactorResults.Fs;
data.baseline=FactorResults.baseline;

[~, name, ~] = fileparts(FactorResults.fileName);
...
if isfield(FactorResults,'taskSpecs')
    data.taskSpecs=FactorResults.taskSpecs;
else
    data.taskSpecs=[];
end

if isfield(FactorResults,'taskNames')
    data.taskNames=FactorResults.taskNames;
else
    data.taskNames=[];
end

if isfield(FactorResults,'taskMeasNames')
    data.taskMeasNames=FactorResults.taskMeasNames;
else
    data.taskMeasNames=[];
end
...
if isfield(FactorResults,'history')
    data.history=FactorResults.history;
end
...
data.video=FactorResults.video;
...
%     eval(['save ' fname 'S' num2str(sub) 'scores.mat scoresMatrix']);
    ...
    paramData=zeros(dataChanNum,dataTimeNum,size(params,2),numSubs,numFacs,dataFreqNum,dataRelNum);
    for iParam = 1:size(params,2)
        index = find(params(:,iParam)~= missingData);	%observations with non-missing params and thus retained for correlation
        correl = corrcoef([scoresMatrix(index,:) params(index,iParam)]);	%column vector of correlations
        ...
        for iCellParam=1:numCells
            for iSubParam=1:numSubs
                if params((iCellParam-1)*numSubs+iSubParam,iParam)==missingData
                    ...
                    for iFac=1:numFacs
                        for iFreq=1:dataFreqNum
                            for iRel=1:dataRelNum
                                ...
                                paramData(iChan,iPoint,iParam,iSub,iFac,iFreq,iRel)=abs(mean(theFacScr(iChan,iPoint,find(~squeeze(paramBadData(iChan,iPoint,:,iSub,1,iFreq,iRel))),iSub,iFac,iFreq,iRel),3))*corrMatrix(corrNum,iFac);
                            end
                        end
                    end
                end
    ...
    EPadd.cellNames=paramNames';
    EPadd.cellTypes=cell(length(paramNames),1);
    EPadd.cellTypes(:)={'CMB'};
    EPadd.data=paramData;
    [data]=ep_addData(data,EPadd,'cells');
end

if addAdds
    if numCells > 1
        disp('Adding combined cells');
        ...
        while sameName
            sameName=0;
            if any(strcmp(PCAnameSuffix,data.cellNames))
                sameName=1;
            end
            if sameName
                suffix=suffix+1;
        ...
        dataOut=ep_combineData(data,'cells',{[],[],[],[],[],[]},[],PCAnameSuffix); %add combined cells
        if EPtictoc.stop;EPtictoc.stop=0;ep('start');return;end
        if isempty(dataOut)
            disp('Error: cancelling addition of cells.')
        else
            data=dataOut;
        end
    end
    ...
    if numFacs > 1
        ...
        dataOut=ep_combineData(data,'factors',{[],[],[],[],[],[]},[],PCAnameSuffix); %add combined factors
        if EPtictoc.stop;EPtictoc.stop=0;ep('start');return;end
        if isempty(dataOut)
            disp('Error: cancelling addition of factors.')
        else
            data=dataOut;
        end
    end
    ...
    if numSubs > 1
        if ~any(strcmp(FactorResults.dataType,{'continuous','single_trial'}))
            ...
            dataOut=ep_combineData(data,'subjects',{[],[],[],[],[],[]},[],PCAnameSuffix); %add combined subjects
            if EPtictoc.stop;EPtictoc.stop=0;ep('start');return;end
            if isempty(dataOut)
                disp('Error: cancelling addition of subjects.')
            else
                data=dataOut;
            end
        end
...
theDescription=['Generated PCA dataset.'];
data.history=ep_addHistory(data.history, EPmain.preferences.records, theDescription, EPmain.handles.hMainWindow, EPmain.preferences, EPmain.preferences.EPver);
