function [FactorResults] = ep_doPCA(PCAmode, ROTATION, ROTOPT, DECOMP, MAT_TYPE, NUM_FAC, theData, LOADING, GAVE, badData, crossFacCof, theAlgorithm)

% ep_doPCA - [FactorResults] = ep_doPCA(PCAmode, ROTATION, ROTOPT, DECOMP, MAT_TYPE, NUM_FAC, theData, LOADING, GAVE, badData, crossFacCof, theAlgorithm) -
...
%       unrotated - Unrotated
%       Varimax - Varimax
%       Promax - Promax (kappa=3 seems to give the best results for ERPs)
%       Infomax - ICA Infomax (if EEGLab is also installed.  ICA is run with
...
%       Extended-Infomax (option is N training blocks, default 1)
%       Quartimax - Quartimax
%       Quartimin - Quartimin
%       Oblimin - Oblimin (option is gamma=0 tends to be recommended)
...
%       JADE
%       SOBI - ROTOPT is the number of correlation matrices to be diagonalized.  {Default: min(100, N/3)}.  Note that for non-ideal data,
%             the authors strongly recommend using at least 100 time delays.
%       fastICA
%       AMICA
%  ROTOPT   : Rotation parameter for those having an optional parameter (Promax and Oblimin and SOBI and extended-Infomax)
%  DECOMP   : Decomposition method for Varimax and Promax rotation options.  'SVD' or 'NIPALS'.
...
%  theData  : Structured EPdata variable with the data and accompanying information (see readData) or else just a data matrix.
%  LOADING	: Loading normalization ('K' Kaiser normalization, 'N' no normalization, 'C' covariance loadings, 'W' Cureton-Mulaik weighting).
%
%  OPTIONAL
...
%  theAlgorithm : which version of the rotation algorithm.
%
%Outputs
%  If no inputs, then a cell array with the supported rotations.
%
...
%
%  Wold, H. (1975). 11 - Path Models with Latent Variables: The NIPALS Approach. 
%     In H. M. Blalock, A. Aganbegian, F. M. Borodkin, R. Boudon, & V. Capecchi (Eds.), Quantitative Sociology (pp. 307?357). Academic Press. https://doi.org/10.1016/B978-0-12-103950-9.50017-4
%
% For information on PCA equations:
% 
...
% Cureton, E. E., & D’Agostino, R. B. (1983). Factor Analysis: An applied approach. Lawrence Erlbaum and Associates.
...
%
% modified 3/14/20 JD
% Added NIPALS decomposition option.
% Optimized some calculations by substituting slashes for inverses and using pairwise correlations rather than the entire data plus factor score matrix for calculating the Factor Structure matrix.
% COR matrix type now allowed for ICA.
%
% bugfix & modified 4/16/20 JD
% Added extended Infomax, JADE, SOBI, and fastICA rotations.
% Turned off binICA since it seems to be slower than matlab code even for linux now.
% Bad observations are output as NaN rather than zeros.
% Function call without inputs yields list of supported rotations.
% Fixed crash for PCA of frequency data.
% Fixed crash for correlation matrix option.
%
% bugfix & modified 4/25/20 JD
% Added AMICA rotation.
% Fixed data being mean-corrected prior to being multiplied by factor scoring coefficients/activation weights and being variance-corrected if the COR option was chosen.
%
% bugfix & modified 5/7/20 JD
% Fixed leftover bug where factor scores were being rescaled and remeaned.
% ICA factors now being generated directly from weights rather than via regression factor scoring method.
%
% bugfix 9/30/20 JD
% Fixed SOBI factor scoring coefficients not being computed correctly (didn't affect anything important).
%
% bugfix 10/8/20 JD
% Fixed crash when conducting spatial PCA on continuous data and there are bad epochs.
% 
...
rotationList={'Varimax';'Promax';'Infomax';'Extended-Infomax';'Quartimax';'Quartimin';'Oblimin';'Crawford-Ferguson';'minimum entropy';'invariant pattern simplicity';'tandem II criterion';'Geomin';'McCammon minimum entropy';'Variable-Oblimin';'JADE';'SOBI';'fastICA';'AMICA';'unrotated'};

if nargin ==0
    FactorResults=rotationList;
    return
end
...
if ~exist('badData','var')
    badData=[];
end

if ~exist('crossFacCof','var')
    crossFacCof=[];
end

if ~exist('theAlgorithm','var')
    theAlgorithm='SAS';
end

if ~any(strcmp(ROTATION,{'Varimax','Promax'}))
    theAlgorithm='';
end

if ~any(strcmp(ROTATION,rotationList))
    error('Rotation not recognized.');
end

if (strcmp(ROTATION,'Promax') && ROTOPT <=0)
    disp('Promax kappa needs to be larger than zero.  Setting it to default value of 3.');
    ROTOPT=3;
end

if strcmp(ROTATION,'SOBI') && strcmp(PCAmode,'temp')
    msg{1}='It is not possible to use SOBI with a temporal mode.';
    [msg]=ep_errorMsg(msg);
    return
end

verList=ver;
% if isempty(find(strcmp('Statistics and Machine Learning Toolbox',{verList.Name}))) && strcmp(theAlgorithm,'SPSS') && any(strcmp(ROTATION,{'Varimax';'Promax'}))
%     msg{1}='Statistics and Machine Learning Toolbox needed to use the SPSS algorithm.';
%     [msg]=ep_errorMsg(msg);
%     return
% end

if isempty(find(strcmp('Deep Learning Toolbox',{verList.Name}))) && strcmp(DECOMP,'NIPALS')
    msg{1}='Deep Learning Toolbox needed to use NIPALS decomposition.';
    [msg]=ep_errorMsg(msg);
    return
end
...
if isstruct(theData)
    ...
    if isempty(badData)
        ...
        switch theData.dataType
            ...
            case 'continuous'
                numEpochs=size(theData.analysis.badTrials,2);
                epochSize=theData.Fs;
                for iEpoch=1:numEpochs
                    if theData.analysis.badTrials(1,iEpoch)
                        badData(:,(iEpoch-1)*epochSize+1:iEpoch*epochSize,:,:,:,:,:)=1;
                        if iEpoch==numEpochs
                            badData(:,iEpoch*epochSize+1:end,:,:,:,:,:)=1; %excess timepoints are appended to the last epoch
                        end
                    end
                end
            otherwise
...
elseif ~strcmp(GAVE,'N')
    msg{1}='GAVE field must equal either "N" or "Y".';
    [msg]=ep_errorMsg(msg);
    return
end
...
elseif strcmp(PCAmode,'asis')
    ...
    msg{1}='PCAmode must be set to either temp or spat or asis';
    [msg]=ep_errorMsg(msg);
    return
end

if strcmp(ROTATION,'SOBI') && (ndims(data)>2)
    data=reshape(data,numChans,[])';
    badData=reshape(badData,numChans,[])';
end
...
if strcmp(ROTATION,'SOBI') && (numPoints<2)
    msg{1}='It is not possible to use SOBI with only one timepoint.';
    [msg]=ep_errorMsg(msg);
    return
end

if strcmp(ROTATION,'SOBI') && (ROTOPT>numPoints)
    msg{1}=['SOBI needs a number of lags (' num2str(ROTOPT) ') less than number of timepoints (' num2str(numPoints) ').'];
    [msg]=ep_errorMsg(msg);
    return
end

% if strcmp(ROTATION,'SOBI')
%     %since SOBI compares across lagged time points, if any timepoints in an epoch is bad, then the entire epoch has to be treated as bad.
%     goodObs=zeros(numObs,1);
%     for iEpoch=1:(numObs/numPoints)
%         goodObs((iEpoch-1)*numPoints+1:iEpoch*numPoints)=all(~any(badData((iEpoch-1)*numPoints+1:iEpoch*numPoints,goodVars)'));
%     end
%     badObs=sparse(~goodObs);
%     goodObs=find(goodObs);
% else
...
numGoodObs=size(data,1);
numGoodVar=size(data,2);

if numGoodVar < NUM_FAC
    msg{1}=['Number of variables (' num2str(numGoodVar) ') is smaller than the number of factors to be retained (' num2str(NUM_FAC) '.'];
    [msg]=ep_errorMsg(msg);
    return
end

if numGoodObs < NUM_FAC
    msg{1}=['Number of observations (' num2str(numGoodObs) ') is smaller than the number of factors to be retained (' num2str(NUM_FAC) ').'];
    [msg]=ep_errorMsg(msg);
    return
end

if rank(data) < NUM_FAC
    msg{1}=['Warning: The rank of the data (' num2str(rank(data)) ') is smaller than the number of factors to be retained (' num2str(NUM_FAC) ').'];
    [msg]=ep_errorMsg(msg);
end
...
% if any(strcmp(ROTATION,{'Infomax','Extended-Infomax'}))
...
%     if exist('ICABINARY','var') == 2 %is there a binary file of runica installed?
%         if any(isspace(pwd))
...
if any(strcmp(MAT_TYPE,{'COV','COR'}))
    data2=data-repmat(varMean,numGoodObs,1);
end
if strcmp(MAT_TYPE,'COR')
    data2=data2*diag(1./varSD);
end

S = ((data2)' * (data2))/(numGoodObs - 1);
...
% if strcmp(ROTATION,'Infomax') && ~strcmp(LOADING,'N')
...
if any(strcmp(ROTATION,{'Infomax','Extended-Infomax'})) && strcmp(MAT_TYPE,'SCP')
    disp(['Matrix type (the ' MAT_TYPE ' option) will instead be treated as ''COV''.']);
end

% if strcmp(ROTATION,'Infomax') && (ROTOPT > 0)
...
if ~isempty(crossFacCof)
    ...
    FacPat = FacStr\FacCor; %compute factor pattern matrix
    ...
    switch DECOMP
        case 'SVD'
            ...
            elseif strcmp(MAT_TYPE,'COR')
                FacScr = (data) * (D\V); %same as above but also corrected for variable variance
            else
                error('Error - matrix type not specified');
            end
            
            ScrDiag = diag(std(FacScr));
            A = Sd\(V * ScrDiag);  %unrotated factor loading matrix
            %formal form A = inv(Sd) * (V * ScrDiag);
        ...
        case 'NIPALS'
            [a,LargeX] = max(std(data));
            RESULTS = nipals_decomp(data,size(data,1), size(data,2), NUM_FAC, 20000, 1e-5, LargeX);
            FacScr = RESULTS.Scores;
            FacStr=zeros(size(data,2),size(FacScr,2));
            for iFac=1:size(FacScr,2)
                for iVar=1:size(data,2)
                    theCor=corrcoef([data(:,iVar),FacScr(:,iFac)]);
                    FacStr(iVar,iFac)=theCor(1,2);
                end
            end
            FacCor = corrcoef(FacScr);
            A = FacStr/FacCor; %compute factor pattern matrix
            scree = RESULTS.Eigenvalues;
            
        otherwise
            error('Error - DECOMP option not recognized.');
    end
    if ~any(strcmp(ROTATION,{'Infomax','Extended-Infomax'}))
        C = sum((A.^2),2)';
        switch LOADING
...
%             end
        ...
        case {'Infomax','Extended-Infomax'}
            ...
            if blocksize < 2
                msg{1}=['Error: Too few observations (' num2str(size(data,1)) ') to conduct Infomax.'];
                [msg]=ep_errorMsg(msg);
                return
            end
            evalc('ep_icadefsWrapper;');
            if any(isspace(pwd)) && exist(ICABINARY,'file') == 2
                disp(['Binary Infomax does not work if any part of the pathname (' pwd ') has a space in it.']);
...
%             ICAsuccess=0;
            ...
            %                 try
            %                     [weights,sphere] = binica(FacScr', 'verbose', 'on','block', blocksize,'filenum',9501);
...
%             if ~ICAsuccess
                try
                    ...
                    switch ROTATION
                        case 'Infomax'
                            [weights,sphere] = el_runica(FacScr', 'verbose', 'off', 'interupt','off','block', blocksize); %by default, sphering but not PCA.
                        case 'Extended-Infomax'
                            [weights,sphere] = el_runica(FacScr', 'verbose', 'off', 'interupt','off','block', blocksize,'extended',ROTOPT); %by default, sphering but not PCA.
                    end
                    warning('on')
                catch ME
                    msg{1}=['Error: computing Infomax resulted in the error ' ME.identifier];
                    msg{2}=ME.message;
                    [msg]=ep_errorMsg(msg);
                    return
                end
...
%             end
            ...
            if NUM_FAC > 1
                if all(diag(weights) == 1) || ~all(isreal(weights))
                    msg{1}='Error: Infomax was unable to converge on a solution.';
                    [msg]=ep_errorMsg(msg);
                    return
                end
            end
            
            FacScr=FacScr*(weights * sphere)';
            FacCof=(weights * sphere * V')';
            FacStr=zeros(size(data,2),size(FacScr,2));
            for iFac=1:size(FacScr,2)
                for iVar=1:size(data,2)
                    theCor=corrcoef([data(:,iVar),FacScr(:,iFac)]);
...
%the assumption that PCA is always used as a first step with ICA, this is the appropriate scree.
        case 'JADE'
            ...
            try
                warning('off')
                weights = jader(FacScr');
                warning('on')
            catch ME
                msg{1}=['Error: computing JADE resulted in the error ' ME.identifier];
                msg{2}=ME.message;
                [msg]=ep_errorMsg(msg);
                return
            end
        ...
        case 'SOBI'
            ICAflag=1;
            if isempty(ROTOPT)
                ROTOPT=min(100,ceil(N/3));
            end
            FacScrAll=nan(numObs,NUM_FAC);
            FacScrAll(goodObs,:)=FacScr;
            FacScrAll=reshape(FacScrAll',NUM_FAC,numPoints,[]);
            try
                warning('off')
                    [weights,~,~]=el_sobi(FacScrAll,NUM_FAC,ROTOPT);
                warning('on')
            catch ME
                msg{1}=['Error: computing SOBI resulted in the error ' ME.identifier];
                msg{2}=ME.message;
                [msg]=ep_errorMsg(msg);
                return
            end
            ...
            FacScr=FacScr*pinv(weights)';
            FacCof=(pinv(weights) * V')';
            FacStr=zeros(size(data,2),size(FacScr,2));
            for iFac=1:size(FacScr,2)
                for iVar=1:size(data,2)
                    theCor=corrcoef([data(:,iVar),FacScr(:,iFac)]);
        ...
        case 'fastICA'
            ...
            try
                warning('off')
                [~, weights] = fastica(FacScr');
                warning('on')
            catch ME
                msg{1}=['Error: computing fastICA resulted in the error ' ME.identifier];
                msg{2}=ME.message;
                [msg]=ep_errorMsg(msg);
                return
            end
            ...
            if size(weights,1)<NUM_FAC
                NUM_FAC=size(weights,1); %fastICA function reduced the number of factors.
            end
            FacScr=FacScr*(weights)';
            FacCof=(weights * V')';
            FacStr=zeros(size(data,2),size(FacScr,2));
        ...
        case 'AMICA'
            if ~isempty(strfind(pwd,' '))
                msg{1}=['The AMICA routine cannot handle paths that have a space in them.  Please change the name of your directory so that there are no spaces:'];
                msg{2}=pwd;
                [msg]=ep_errorMsg(msg);
                return
            end            
            ...
            try
                warning('off')
                [weights,sphere,mods] = runamica15(FacScr','max_iter',1000,'max_threads',16); %crashes if sphering turned off
                warning('on')
            catch ME
                msg{1}=['Error: computing AMICA resulted in the error ' ME.identifier];
                msg{2}=ME.message;
                [msg]=ep_errorMsg(msg);
                return
            end
    ...
    if ~ICAflag
        switch LOADING
            ...
            case 'N'
            case 'W'
                FacPat = diag(w)\FacPat;
                FacPat = diag(facReflect)*FacPat;
                FacPat = diag(sqrt(C)) * FacPat;  %renormalize factor loadings by original communalities
%                 FacStr = diag(w)\FacPat;
%                 FacStr = diag(facReflect)*FacPat;
    ...
    if strcmp(ROTATION,'Promax')
        [FacPat, FacCor] = ep_doPromax(FacPat, ROTOPT,theAlgorithm); %Only apply loading weighting to the Varimax step by doing this step after the loadings have been renormalized
    ...
    if ~ICAflag
        FacCof=pinv(Sd*FacPat)';
        FacScr=data*FacCof;
    end
    
    %The previously implemented method (up through version 2.88) was the regression method.
    %The regression method is FacScr=data*invR*FacStr; Gorsuch (1983) p. 262 and is the most common approach for Principal Axes (that is, Factor Analysis in the more narrow sense).
    %The imprecision manifests, among other ways, in that the resulting factor scores are not actually entirely uncorrelated even for Varimax.  Gorsuch (1983) p. 263.
    
    %The PCA equation of FacScr=data*pinv(FacPat)' is exact but only applies when the FacPat matrix is orthogonal.  Cureton & D'Agostino (1983) p. 318
    %While the Factor Pattern matrices for Varimax and Promax are not exactly orthogonal, I'm finding that it seems to be good enough so it is now used as of 2.89.
end
...
if LargestCom ~=0 && LargestLoading~=0
    ...
    FacScr=FacScr/diag(std(FacScr)); %Standardize factor scores, not mean corrected.

    Var=Sd.^2;

    %calculate communalities (equivalent to calculating R-Squared in multiple regression).

    Comm=sum((Var*FacPat.*FacStr),2)/sum(diag(Var)); % Cohen & Cohen (1983) p. 100
...
%     disp(['Amount of variance accounted for by the ' theRotationName2 ' solution is ' sprintf('%3.2f%%.',100*sum(Comm,1))]);
    ...
    %between the factor and the variable.  Cohen & Cohen (1983) p. 470
    
    facVarQ=sum(Var*(FacPat/diag(sqrt(diag(inv(FacCor))))).^2)/sum(diag(Var));
...
FactorResults.FacScr=nan(size(badData,1),size(FacScr,2));
...
FactorResults.decomp=DECOMP;
FactorResults.theAlgorithm=theAlgorithm;
...
if isstruct(theData)
    ...
    FactorResults.sessNames=theData.sessNames;
    ...
    FactorResults.sessNums=theData.sessNums;
    ...
    if isfield(theData,'history')
        FactorResults.history=theData.history;
    end
    ...
    if isfield(theData,'video')
        FactorResults.video=theData.video;
    end
