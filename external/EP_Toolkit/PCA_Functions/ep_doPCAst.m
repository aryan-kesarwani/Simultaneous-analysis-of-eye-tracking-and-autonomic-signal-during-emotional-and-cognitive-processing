function [FactorResultsST] = ep_doPCAst(FactorResults, ROTATION, ROTOPT, DECOMP, MAT_TYPE, NUM_FAC, LOADING, PCAmode, crossVerify, theAlgorithm)
% ep_doPCAst - [FactorResultsST] = ep_doPCAst(FactorResults, ROTATION, ROTOPT, DECOMP, MAT_TYPE, NUM_FAC, LOADING, PCAmode, crossVerify, theAlgorithm);
%Inputs
...
%  .decomp      : Decomposition option from first step
%  .decomp2     : Decomposition option from second step
...
%
% modified 3/13/20 JD
% Added support for decomp option.
%
% modified 8/6/22 JD
...
if ~isfield(FactorResults,'decomp')
    FactorResults.decomp='SVD';
end
...
if isfield(FactorResults,'PCAmode')
    ...
    if isfield(FactorResults,'PCAmode2')
        ...
        badObs=FactorResults.badObsST(:,1);
...
for factor = 1:size(FacScr,2)	%input factors
    ...
    if isfield(FactorResults,'dataType') %EP file format
        ...
        theData.sessNames=FactorResults.sessNames;
        ...
        theData.sessNums=FactorResults.sessNums;
        ...
        if isfield(FactorResults,'taskSpecs')
            theData.taskSpecs=FactorResults.taskSpecs;
        end
        if isfield(FactorResults,'taskNames')
            theData.taskNames=FactorResults.taskNames;
        end
        if isfield(FactorResults,'taskMeasNames')
            theData.taskMeasNames=FactorResults.taskMeasNames;
        end
    ...
    [stFactorResults] = ep_doPCA('asis', ROTATION, ROTOPT, DECOMP, MAT_TYPE, NUM_FAC, theData, LOADING, 'N', badData, theCrossVerify, theAlgorithm);
...
if isfield(FactorResults,'PCAmode2')
    ...
    for factor1=1:FactorResults.numFacs
        for factor2=1:FactorResults.numFacs2
            for factor3=1:NUM_FAC
                facNames{(factor1-1)*FactorResults.numFacs2*NUM_FAC+(factor2-1)*NUM_FAC+factor3,1}=sprintf('%s%s%s%s%s%s',label1,sprintf(['%0' num2str(factor1Digits) 'd'],factor1),label2,sprintf(['%0' num2str(factor2Digits) 'd'],factor2),label3,sprintf(['%0' num2str(factor3Digits) 'd'],factor3));
                facTypes{(factor1-1)*FactorResults.numFacs2*NUM_FAC+(factor2-1)*NUM_FAC+factor3,1}='SGL';
            end
        end
    end
    ...
    FactorResultsST.decomp=FactorResults.decomp;
    FactorResultsST.decomp2=FactorResults.decomp2;
    FactorResultsST.decomp3=DECOMP;

    FactorResultsST.theAlgorithm=FactorResults.theAlgorithm;
    FactorResultsST.theAlgorithm2=FactorResults.theAlgorithm2;
    FactorResultsST.theAlgorithm3=theAlgorithm;
    ...
    FactorResultsST.decomp=FactorResults.decomp;
    FactorResultsST.decomp2=DECOMP;
    FactorResultsST.theAlgorithm=FactorResults.theAlgorithm;
    FactorResultsST.theAlgorithm2=theAlgorithm;
    FactorResultsST.numFacs=FactorResults.numFacs;
...
if isfield(FactorResults,'dataType') %EP file format
    ...
    FactorResultsST.sessNums=FactorResults.sessNums;
    ...
    FactorResultsST.sessNames=FactorResults.sessNames;
    FactorResultsST.facTypes=FactorResults.facTypes;
    ...
    if isfield(FactorResults,'history')
        FactorResultsST.history=FactorResults.history;
    end
    ...
    if isfield(FactorResults,'video')
        FactorResultsST.video=FactorResults.video;
    end
    if isfield(FactorResults,'avgNum')
    ...
    if isfield(FactorResults,'taskSpecs')
        FactorResultsST.trialSpecNames=FactorResults.taskSpecs;
    end
    if isfield(FactorResults,'taskNames')
        FactorResultsST.trialSpecNames=FactorResults.taskNames;
    end
    if isfield(FactorResults,'taskMeasNames')
        FactorResultsST.trialSpecNames=FactorResults.taskMeasNames;
    end
end
