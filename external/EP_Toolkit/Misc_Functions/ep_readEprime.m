...
[fid,msg]=fopen(fname,'r','n',textFormat);
warning('on','MATLAB:iofun:UnsupportedEncoding')

if strcmp(textFormat,'UTF-16LE')
    %using workaround from https://www.mathworks.com/matlabcentral/answers/270857-how-to-read-16-bit-text-with-matlab
    fread(fid, 2, '*uint8');   %adjust the 2 to fit the UTF encoding
    filecontent = fread(fid, [1 inf], '*char');
    fclose(fid);
    C = textscan(filecontent, '%[^\n]', 'delimiter','\n');
else
    txt=fscanf(fid,'%c');
    C=textscan(txt,'%[^\n]','delimiter','\n');
    fclose(fid)
