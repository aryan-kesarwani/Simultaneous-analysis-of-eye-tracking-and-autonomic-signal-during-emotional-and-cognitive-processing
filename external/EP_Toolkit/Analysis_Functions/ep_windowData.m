function outputdata=ep_windowData(EPdata, chanGrp, inputCells, outCellNames, subjectSpecsIndex, startwin, endwin, measure, fname, factor, startfreq, endfreq, FFTunits, chanColl, missingData, peakWidth, numberType, fileList)

%  outputdata=ep_windowData(EPdata, chanGrp, inputCells, outCellNames, subjectSpecsIndex, startwin, endwin, measure, fname, factor, startfreq, endfreq, FFTunits, chanColl, missingData, peakWidth, numberType, fileList);
%
%  Outputs text file suitable for ANOVA.  Each line is a subject.  As for columns, session names will vary slowest, then cells, then chan groups will
...
%  Starts with eight header lines: name of dataset, window, type of measure, channel group, factor if any, session names, cell names, and channel area
%  names.
%  For single-trial data, each line will be a trial, the columns will be cells, and no specs will be included.
%  For meanERA data, each line will be a trial, the columns will be the subject ID, the condition, a column for each Area, session if the data are multi-session, and then the user-selected subject specs.
%
%Inputs:
...
%
% inputCells : Cell array of conditions included in analysis (output cells, each containing array of input cells, or trials for single-trial data)
% outCellNames: Cell array of the output cell names
% subjectSpecsIndex : Logical vector array of which specs to include with the output measures
% startwin   : Start of time window in samples (1-based).
% endwin	 : End of time window in samples.
% measure	 : Type of measure for windows: mean, minpeak, maxpeak, minlatency, maxlatency, mincentroid, maxcentroid, meanStdVar, behavioral, task, BOSCepisodes, meanSME, meanNoise, meanERA.
...
% chanColl:   : Collapse channels then measure (1) or measure then collapse (2)
...
% numberType  : Type of data to output: 'imag', 'real', or 'normal' for EEG data or 'RT' or 'ACC' for behavioral data or name of task measure for task.
% fileList    : For meanERA, the list of single-trial files to be loaded in.
%
%Outputs:
...
%
% bugfix 10/25/19 JD
% Fixed crash with maxPeaks option when the epoch is only one sample long, as in temporal PCA data.
%
...
%
% modified 2/25/20 JD
% Added BOSCepisodes option.
%
% bugfix 11/7/20 JD
% Fixed subject labels always being output in alphabetical order, resulting in mismatch with the voltage data if the labels were not in alphabetical order.
%
% bugfix 11/21/20 JD
% Fixed crash when windowing multi-session data.
%
% bugfix 3/12/21 JD
% Fixed single-trial windowed output sometimes set to zero rather than missing data number for bad data trials.
%
% modified 11/8/21 JD
% Added meanNoise option.
%
% modified 12/18/21 JD
% Added meanERA option.
%
% bugfix 1/20/23 JD
% Fixed subject specs not matching up correctly when there are multiple sessions per subject.
%
% modified & bugfix 4/11/25 JD
% Implemented version 3 output with origin cell names.
% Fixed crash when measuring coherences with more than one channel in an Area.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isempty(EPdata.covAVE) && strcmp('meanStdVar',measure)
    msg{1}='Error: Cannot calculate mean standard variance as the data is not available.';
    [msg]=ep_errorMsg(msg);
    return
end

if isempty(EPdata.noise) && strcmp('meanNoise',measure)
    msg{1}='Error: Cannot calculate mean noise as the data is not available.';
    [msg]=ep_errorMsg(msg);
    return
end
...
if strcmp(measure,'meanERA')
    EPdataAVG=EPdata;
    inputCellsERA=inputCells;
end
...
elseif strcmp(measure,'task')
    numAreas=1;
    chanArea=1;
    chanAreaName{1}=numberType;
    inputCells=cell(length(EPdata.taskNames),1);
    outCellNames=EPdata.taskNames;
    
else
    %determine number of channel areas and which channels are assigned to them.
    numAreas=0;
    for iArea=1:length(chanGrp.areaName)-1
        chanArray=find(chanGrp.channel == iArea);
        if ~isempty(chanArray)
            numAreas=numAreas+1;
            chanArea{numAreas}=chanArray;
            chanAreaName{numAreas}=chanGrp.areaName{iArea};
        end
    end
end

if ~isempty(EPdata.sessNums) && (length(unique(EPdata.sessNums))>1)
    sessList=unique(EPdata.sessNums(EPdata.sessNums~=0));
    numSess=length(sessList);
    sessNames=cell(numSess,1);
    for iSess=1:numSess
        sessNames{iSess}=EPdata.sessNames{sessList(iSess)};
    end
    %if there is no session data, then duplicate subject names are ignored, otherwise are interpreted as being from different sessions.
    subList=unique(EPdata.subNames);
    subIndex=zeros(numSubs,1);
    sessIndex=zeros(numSubs,1);
    for iSub = 1:numSubs
        subIndex(iSub)=find(strcmp(EPdata.subNames{iSub},subList));
        if EPdata.sessNums(iSub)~=0
            sessIndex(iSub)=find(EPdata.sessNums(iSub)==sessList);
        else
            sessIndex(iSub)=0;
        end
    end
    numUniqueSubs=length(subList);
    for iSub = 1:numSubs
        if length(intersect(find(subIndex(iSub)==subIndex),find(sessIndex(iSub)==sessIndex)))>1
            msg{1}=['Problem with subject ' EPdata.subNames{iSub} '.'];
            msg{2}='When windowing data with multiple sessions, each subject name can only appear once in a given session.';
            [msg]=ep_errorMsg(msg);
            return
        end
    end
    ...
    sessList=[];
    numSess=1;
    numUniqueSubs=numSubs;
    subList=EPdata.subNames;
end

numOutCells=length(outCellNames);

ep_tictoc;if EPtictoc.stop;return;end
%construct array of windowed measures
if strcmp(EPdata.dataType,'single_trial') || strcmp(measure,'meanERA')
    outputdata = ones(sum(cellfun(@length,inputCells)),numAreas)*missingData;
    trialCount=zeros(numUniqueSubs,numOutCells);
    trialLabels=cell(size(outputdata,1),2);
else
    outputdata = ones(numUniqueSubs,(numSess*numAreas*length(inputCells)))*missingData;
end
multChansFlag=0;
for iSub = 1:numSubs
    if ~isempty(sessList)
        theSess=sessIndex(iSub);
        theSub=subIndex(iSub);
    else
        theSess=1;
        theSub=iSub;
    end
    if strcmp(measure,'meanERA')
        theFile=fileList{iSub};
        load('-mat', theFile, 'EPdata');
        for iCell = 1:length(inputCellsERA)
            inputCells{iCell}=find(ismember(EPdata.cellNames,EPdataAVG.cellNames(inputCellsERA{iCell})));
        end
    end
    for theOutCell = 1:numOutCells
        for iArea = 1:numAreas
            theData = 0;
            theTrialData=nan(length(inputCells{theOutCell}),1);
            count = 0;
            if strcmp(measure,'task')
                theOkayCells=theOutCell;
            else
                if strcmp(EPdata.dataType,'average')
                    theOkayCells=inputCells{theOutCell}(find(EPdata.avgNum(iSub,inputCells{theOutCell})>=0));
                else
                    theOkayCells=inputCells{theOutCell}(find(EPdata.analysis.badTrials(1,inputCells{theOutCell})==0));
                end
            end
                ...
                for theInCell = 1:length(theOkayCells)
                    ...
                    if strcmp(measure,'behavioral')
                        if strcmp(numberType,'RT')
                            theData=EPdata.trialSpecs{theCell,find(strcmp('RT',EPdata.trialSpecNames)),iSub};
                        else
                            theData=EPdata.trialSpecs{theCell,find(strcmp('ACC',EPdata.trialSpecNames)),iSub};
                        end
                        count=1;
                    elseif strcmp(measure,'task')
                        theData=EPdata.taskSpecs(iSub,theCell,find(strcmp(numberType,EPdata.taskMeasNames)));
                        count=1;
                    else
                        if strcmp(EPdata.dataType,'average')
                            goodChans=find(~isnan(EPdata.analysis.badChans(iSub,theCell,:)));
                        else
                            goodChans=find(EPdata.analysis.badChans(1,theCell,:) >= 0);
                        end
                        chanList=intersect(chanArea{iArea},goodChans);
                        if ~isempty(chanList)
                            chanLength=1;
                            ...
                            if strcmp(EPdata.dataType,'average')
                                goodRelChans=find(squeeze(~isnan(EPdata.analysis.badChans(iSub,theCell,:))));
                            else
                                goodRelChans=find(squeeze((EPdata.analysis.badChans(1,theCell,:)~=-1)));
                            end
                            ...
                            for theChan = 1:chanLength
                                if chanColl ==1 %collapse channels together prior to measuring
                                    ...
                                    for theChanColl=chanList(:)'
                                        if strcmp('meanStdVar',measure)
                                            if size(EPdata.covAVE,7)==1
                                                chanEpoch=sqrt(EPdata.covAVE(theChanColl,startwin:endwin,theCell,iSub,factor,startfreq:endfreq,1));
                                            else
                                                chanEpoch=sqrt(EPdata.covAVE(theChanColl,startwin:endwin,theCell,iSub,factor,startfreq:endfreq,theChanColl));
                                            end
                                        elseif strcmp('meanNoise',measure)
                                            chanEpoch=EPdata.noise(theChanColl,startwin:endwin,theCell,iSub,factor,startfreq:endfreq,1);
                                        elseif strcmp(measure,'meanERA')
                                            chanEpoch=ep_expandFacs(EPdata,theChanColl,startwin:endwin,theCell,1,factor,startfreq:endfreq,relChans);
                                            ep_tictoc;if EPtictoc.stop;return;end
                                        else
                                            chanEpoch=ep_expandFacs(EPdata,theChanColl,startwin:endwin,theCell,iSub,factor,startfreq:endfreq,relChans);
                                            ep_tictoc;if EPtictoc.stop;return;end
                                        end
                                        if numRels
                                            if multChansFlag
                                                chanEpoch=mean(chanEpoch,7);
                                            else
                                    ...
                                    if strcmp('meanStdVar',measure)
                                        if size(EPdata.covAVE,7)==1
                                            chanEpoch=sqrt(EPdata.covAVE(chanList(theChan),startwin:endwin,theCell,iSub,factor,startfreq:endfreq,1));
                                        else
                                            chanEpoch=sqrt(EPdata.covAVE(chanList(theChan),startwin:endwin,theCell,iSub,factor,startfreq:endfreq,chanList(theChan)));
                                        end
                                    elseif strcmp('meanNoise',measure)
                                        chanEpoch=EPdata.noise(chanList(theChan),startwin:endwin,theCell,iSub,factor,startfreq:endfreq,1);
                                    elseif strcmp(measure,'meanERA')
                                        chanEpoch=ep_expandFacs(EPdata,chanList(theChan),startwin:endwin,theCell,1,factor,startfreq:endfreq,relChans);
                                        ep_tictoc;if EPtictoc.stop;return;end
                                    else
                                        chanEpoch=ep_expandFacs(EPdata,chanList(theChan),startwin:endwin,theCell,iSub,factor,startfreq:endfreq,relChans);
                                        ep_tictoc;if EPtictoc.stop;return;end
                                    end
                                    if numRels
                                        if multChansFlag
                                ...
                                if ~isempty(EPdata.freqNames) && any(ismember(EPdata.chanTypes(chanList),{'EEG','REG'}))
                                ...
                                switch measure
                                    case {'mean','meanSME', 'meanNoise', 'meanERA'}
                                        theData = theData + mean(epoch,2);
                                    case 'minpeak'
                                        if length(epoch) > 2
                                    ...
                                    case 'meanStdVar'
                                        theData = theData + mean(epoch,2);
                                    case 'BOSCepisodes'
                                        theData = theData + length(find(diff([0 (epoch~=0)])>0));
                                    otherwise
                                        msg{1}='Measure should be one of the following options: mean, minpeak, maxpeak, minlatency, maxlatency, mincentroid, maxcentroid, minHzPeak, maxHzPeak, minHzLatency, maxHzLatency, minHzCentroid, maxHzCentroid, BOSCepisodes';
                                        [msg]=ep_errorMsg(msg);
                                        return
                                end
                                count = count +1;
                    ...
                    if strcmp(EPdata.dataType,'single_trial')
                        theTrial=find(theCell==inputCells{theOutCell});
                        theTrialData(theTrial)=theData/count;
                        theData=0;
                        count=0;
                    end
                end
            ...
            if (count == 0 && ~strcmp(EPdata.dataType,'single_trial'))
                outputdata(theSub,(theSess-1)*(numOutCells*numAreas)+(theOutCell-1)*numAreas+iArea) = missingData;
            else
                if strcmp(EPdata.dataType,'single_trial')
                ...
                if any(strcmp(measure,{'mean','minpeak','maxpeak','meanSME', 'meanERA'})) && numFreqs && ~numRels
                ...
                if strcmp(EPdata.dataType,'single_trial')
                    trialSum=sum(trialCount,'all');
                    if isempty(theData)
                        outputdata(trialSum+1:trialSum+length(inputCells{theOutCell}),iArea) = missingData;
                    else
                        outputdata(trialSum+1:trialSum+length(inputCells{theOutCell}),iArea) = theData;
                    end
                    [trialLabels{trialSum+1:trialSum+length(inputCells{theOutCell}),1}]=deal(outCellNames{theOutCell});
                    [trialLabels(trialSum+1:trialSum+length(inputCells{theOutCell}),2)]=deal(num2cell([1:length(inputCells{theOutCell})]));
                else
                    if isempty(theData)
                        outputdata(theSub,(theSess-1)*(numOutCells*numAreas)+(theOutCell-1)*numAreas+iArea) = missingData;
                    else
                        outputdata(theSub,(theSess-1)*(numOutCells*numAreas)+(theOutCell-1)*numAreas+iArea) = theData;
                    end
                end
            end
        end
        if strcmp(EPdata.dataType,'single_trial')
            trialCount(theSub,theOutCell)=length(inputCells{theOutCell});
        end
    end
end

if strcmp(measure,'meanERA')
    %ERA software does not have a missing data code so instead drop trial if any of the area measures are NaN.
    dropTrials=find(any(isnan(outputdata),2));
    if ~isempty(dropTrials)
        disp(['Dropping ' num2str(length(dropTrials)) ' trials.'])
        cummCount=zeros(numel(trialCount),1);
        trialCount2=trialCount';
        for iCount=1:numel(trialCount)
            cummCount(iCount)=sum(trialCount2(1:iCount));
        end
        for iDrop=1:length(dropTrials)
            trialCount2(min(find(cummCount>=dropTrials(iDrop))))=trialCount2(min(find(cummCount>=dropTrials(iDrop))))-1; 
        end
        trialCount=trialCount2';
        outputdata(dropTrials,:)=[];
        if sum(trialCount,'all') ~= size(outputdata,1)
            msg{1}=['Aborting due to programming error.  Contact the programmer for help.'];
            [msg]=ep_errorMsg(msg);
            return
        end
    end
...
if any(outputdata==missingData,'all') && any(strcmp(measure,{'minpeak';'maxpeak';'minlatency';'maxlatency'}))
    disp('You got missing numbers.  It may be that you asked for a peak/dip measure and there was no peak/dip in the window.  Try the opposite measurement.')
end

ep_tictoc;if EPtictoc.stop;return;end
if any(strcmp(measure,{'mean','minpeak','maxpeak','meanSME', 'meanERA'})) && numFreqs && ~strcmp(measure,'behavioral')
    if isscalar(EPdata.freqNames)
...
switch numberType
    ...
    otherwise
        if strcmp(measure,'task')
            fname=[fname(1:end-4) '_' numberType '.txt'];
        else
...
if strcmp(measure,'meanERA')
    %file header
    fprintf(outFID,'ID\tCell');
    for iArea=1:length(chanAreaName)
        fprintf(outFID,'\t%s',chanAreaName{iArea});
    end
    if ~isempty(sessNames)
        fprintf(outFID,'\tSession');
    end
    for iSpec = 1:length(subjectSpecsIndex)
        fprintf(outFID,'\t%s',EPdataAVG.subjectSpecNames{subjectSpecsIndex(iSpec)});
    end
    fprintf(outFID,'\n');
    %file data
    theSub=1;
    theCell=1;
    for iSub=1:size(outputdata,1)
        if (sum(trialCount(1:theSub-1,:),'all')+sum(trialCount(theSub,1:theCell))) < iSub
            if theCell==numOutCells
                theSub=theSub+1;
                theCell=1;
            else
                theCell=theCell+1;
        ...
        if ~isempty(sessList)
            theSess=sessIndex(theSub);
            theSubNum=subIndex(theSub);
        else
            theSess=1;
            theSubNum=theSub;
        end
        fprintf(outFID,[subList{theSubNum} '\t']);
        fprintf(outFID,'%s',outCellNames{theCell});
        for iArea = 1:size(outputdata,2)
            fprintf(outFID,'\t%f',outputdata(iSub,iArea));
        end
        if ~isempty(sessNames)
            fprintf(outFID,'\t%s',sessNames{theSess});
        end
        for iSpec = 1:length(subjectSpecsIndex)
            theSpec=EPdataAVG.subjectSpecsIndex{theSub,subjectSpecsIndex(iSpec)};
    ...
    %file header
    if strcmp(measure,'behavioral')
        fprintf(outFID,'EP Toolkit Version: %s\n',EP_VER);
        fprintf(outFID,'%s\n','behavioral');
        fprintf(outFID,'\n');
        fprintf(outFID,'\n');
        fprintf(outFID,'\n');
        ...
        if (startwin == endwin) && (startfreq == endfreq) && strcmp(measure,'mean')
            if isempty(EPdata.freqNames)
                fprintf(outFID,'%s\n',['One sample of ' dataType]);
            else
                if isempty(EPdata.timeNames)
                    fprintf(outFID,'%s\n',['One frequency bin of ' dataType]);
                else
                    fprintf(outFID,'%s\n',['One frequency wavelet of ' dataType]);
                end
            end
        elseif strcmp(measure,'meanSME')
            fprintf(outFID,'SME\n');
        else
            fprintf(outFID,'%s\n',[measure ' of ' dataType]);
        end

        fprintf(outFID,'Channel Group: %s\n',chanGrp.name);
        if isempty(EPdata.facNames)
            fprintf(outFID,'\n');
        else
            fprintf(outFID,'Factor: %s (%d)\n',EPdata.facNames{factor},factor);
        end
    end
    ...
    if strcmp(EPdata.dataType,'single_trial')
        fprintf(outFID,'\n');
        for iArea = 1:length(chanArea)
            fprintf(outFID,'%s\t',chanAreaName{iArea});
        end
        fprintf(outFID,'\tcell\ttrial\n');
        ...
        for iTrial=1:size(outputdata,1)
            ...
            fprintf(outFID,'\t%4.0f\n',trialLabels{iTrial,2});
        end
    else
        %provide cell names contributing to each column of data.
        for iSess = 1:numSess
            for iCell = 1:numOutCells
                for iArea = 1:numAreas
                    dataCells='';
                    for iRow=1:length(inputCells{iCell})
                        dataCells=[dataCells '@' cellNames{inputCells{iCell}(iRow)}];
                    end
                    fprintf(outFID,'%s\t',dataCells);
                end
            end
        end
        fprintf(outFID,'\n');
        ...
        if ~isempty(sessNames)
            for iSess = 1:numSess
                for iCell = 1:numOutCells
                    for iArea = 1:length(chanArea)
                        fprintf(outFID,'%s\t',sessNames{iSess});
                    end
                end
            end
            subSpecs=cell(length(subList),size(EPdata.subjectSpecs,2));
            for iSub=1:size(outputdata,1)
                subSessList=find(strcmp(subList{iSub},EPdata.subNames));
                subSpecs(iSub,:)=EPdata.subjectSpecs(subSessList(1),:);
                if length(subSessList) > 1
                    mismatchFlag=0;
                    for iSess=2:length(subSessList)-1
                        if ~isequal(EPdata.subjectSpecs(subSessList(1),subjectSpecsIndex),EPdata.subjectSpecs(subSessList(iSess),subjectSpecsIndex))
                            mismatchFlag=1;
                        end
                    end
                    if mismatchFlag
                        disp(['Warning: The specs of the subject ' subList{iSub} ' do not match across its sessions.'])
                    end
                end
            end
        else
            subSpecs=EPdata.subjectSpecs;
        end

        for theSpec = 1:length(subjectSpecsIndex)
            fprintf(outFID,'\t');
        end
        fprintf(outFID,'\n');
        ...
        for iSess = 1:numSess
            for iCell = 1:numOutCells
                for iArea = 1:length(chanArea)
                    fprintf(outFID,'%s\t',outCellNames{iCell});
                end
            end
        end
        for iSpec = 1:length(subjectSpecsIndex)
            fprintf(outFID,'%s',EPdata.subjectSpecNames{subjectSpecsIndex(iSpec)});
            if iSpec < length(subjectSpecsIndex)
                fprintf(outFID,'\t');
            end
        end
        fprintf(outFID,'\n');
        ...
        %file data
        for iSub=1:size(outputdata,1)
            for iCell = 1:size(outputdata,2)
                fprintf(outFID,'%f\t',outputdata(iSub,iCell));
            end
            for iSpec = 1:length(subjectSpecsIndex)
                theSpec=subSpecs{iSub,subjectSpecsIndex(iSpec)};
                if isnumeric(theSpec)
                    theSpec=num2str(theSpec);
                end
                fprintf(outFID,'%s\t',theSpec);
            end
            fprintf(outFID,[subList{iSub} '\n']);
        end
    end
end
...
if ~strcmp(measure,'meanERA')
    disp(['You have missing numbers set to be: ' num2str(missingData)])
    disp(['The number of missing numbers was: ' num2str(length(find(outputdata==missingData)))])
end