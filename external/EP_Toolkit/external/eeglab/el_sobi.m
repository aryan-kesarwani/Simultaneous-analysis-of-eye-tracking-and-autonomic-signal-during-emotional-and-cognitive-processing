function [H,S,D]=el_sobi(X,n,p)
% el_sobi() - Second Order Blind Identification (SOBI) by joint diagonalization of
...
%
% 4/14/20 JD
% Added noncov subfunction to accommodate NaN values and commented out the whitening and mean correction code and modified computation of M matrix to allow for skipping over NaN trials.
% Data needs to come in already standardized and decorrelated.
...
%whitening code commented out because EP Toolkit doPCA function already
%does so and is written to handle NaN values.  JD
%
...
%         if t == 1
            ...
            covMat = X(:,k:N,t)*X(:,1:N-k+1,t)'/(N-k+1);
%         else
%             %Rxp=Rxp+X(:,k:N,t)*X(:,1:N-k+1,t)'/(N-k+1)/ntrials;
%             covMat = Rxp+nancov(X(:,k:N,t),X(:,1:N-k+1,t)')/ntrials; %JD
%         end
        if ~any(any(isnan(covMat))) %JD
            Rxp=Rxp+covMat; %JD
            theCount=theCount+1; %JD
        end
    end
    Rxp=Rxp/theCount; %JD
...
%
% H = pinv(Q)*V; %commented out JD
H=V; %added empty default outputs.  JD
S=[];
D=[];
